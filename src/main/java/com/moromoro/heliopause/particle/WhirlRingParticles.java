...
public class WhirlRingParticles extends TextureSheetParticle {
    ...
    //軌道要素を設定する
    public void setOrbitalElements(float axisPitch, float axisYaw, float axisRoll, float radius, float centripetalForce){
    ...
    public void setPixelBasedSize(float quadPixelSize){
        this.quadSize= (float) (Math.ceil(4 * quadPixelSize)/16f);
    }
    ...
    public void render(@NotNull VertexConsumer buffer, @NotNull Camera camera, float partialTicks) {
        ...
        float quadLengthHalf = (quadHalf + getOrbitSpeed()*10f)*this.alpha;
        float radius = this.orbitRadius/quadSize;
        float zError = 0.1f*quadSize;
        //オモテ用
        Vector3f [] overVertexPosArray = new Vector3f[]{
                new Vector3f(-quadLengthHalf, -quadHalf + radius, zError),
                new Vector3f(-quadLengthHalf, quadHalf + radius, zError),
                new Vector3f(quadLengthHalf, quadHalf + radius, zError),
                new Vector3f(quadLengthHalf, -quadHalf + radius, zError)
        };
    ...
    public void tick() {
        ...
        //小さくて遠いなら消す
        if((this.cameraDistance/Math.pow(this.quadSize,0.2f))>80){
            this.remove();
        }
